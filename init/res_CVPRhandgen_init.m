function net = res_CVPRhandgen_init(m, varargin)
switch m,
...
block = dagnn.Conv('size',  [1 1 ninputs 512], 'hasBias', true, ...
    'stride', 2, 'pad', 0);

lName = 'convfc1';
net.addLayer(lName, block, 'image', lName, {[lName '_f'], [lName '_b']});

add_layer_bn(net, 512, lName, 'bnfc1', 0.1);
...
block = dagnn.Conv('size',  [1 1 512 1024], 'hasBias', true, ...
    'stride', 1, 'pad', 0);

lName = 'convfc2';
net.addLayer(lName, block, 'relufc1', lName, {[lName '_f'], [lName '_b']});

add_layer_bn(net, 1024, lName, 'bnfc2', 0.1);
block = dagnn.ReLU('leak',0.2);
...
lName = 'convTfc0';
net.addLayer(lName, block, 'relufc2', lName, {[lName '_f'], [lName '_b']});
