function net = res_CVPRUarch_init(m, varargin)
switch m,
...
net.meta.trainOpts.learningRate = [0.001*ones(1,5) 0.0001*ones(1,5) 0.0001*ones(1,5) 0.00005*ones(1,5) 0.00001*ones(1,5)] ;
net.meta.trainOpts.numEpochs = numel(net.meta.trainOpts.learningRate);

% First conv layer
nfilters=32;
block = dagnn.Conv('size',  [3 3 ninputs nfilters], 'hasBias', true, ...
                   'stride', 2, 'pad', [1 1 1 1]);%64x64x16
lName = 'conv0';
net.addLayer(lName, block, 'image', lName, {[lName '_f'], [lName '_b']});

add_layer_bn(net, nfilters, lName, 'bn0', 0.1);
block = dagnn.ReLU('leak',0);
net.addLayer('relu0',  block, 'bn0', 'relu0');


info.lastNumChannel = nfilters;
info.lastIdx = 0;

% Three groups of layers
info = add_group(opts.networkType, net, n, info, 3, 64, 2, opts);%32x32x16
info = add_group(opts.networkType, net, n, info, 3, 96, 2, opts);%16x16x32
info = add_group(opts.networkType, net, n, info, 3, 128, 2, opts); %8x8x64
%info = add_group(opts.networkType, net, n, info, 3, 128, 2, opts); %

% Three groups of transpose layers

%info = add_groupT(opts.networkType, net, n, info, 3, 128, 0.5, opts);%
info = add_groupT(opts.networkType, net, n, info, 3, 128, 0.5, opts);%16x16x64
info = add_groupT(opts.networkType, net, n, info, 3, 96, 0.5, opts); %32x32x32
info = add_groupT(opts.networkType, net, n, info, 3, 64, 0.5, opts); %64x64x16
info = add_groupT(opts.networkType, net, n, info, 3, 32, 0.5, opts); %128x128x8
...
idx_down=zeros(length(net.vars),1);
for kk=1:length(net.vars)
    m=strfind(net.vars(kk).name,'down');
    if ~isempty(m)
        idx_down(kk)=1;
    end
end

idxdown=find(idx_down);


idx_up=zeros(length(net.vars),1);
for kk=1:length(net.vars)
    m=strfind(net.vars(kk).name,'up');
    if ~isempty(m)
        idx_up(kk)=1;
    end
end
...
idxall=[idxdown,idxup];

for kk=1:size(idxall,1)
    
    block =  dagnn.Concat('dim',3) ;

    input_cc1=net.vars((idxall(kk,1)-1)).name;
    input_cc2=net.vars((idxall(kk,2)-1)).name;
    input_cc={input_cc1,input_cc2};
    output_cc=[input_cc1,'_',input_cc2];
    concat_name=['concat_',num2str(kk)];
    net.addLayer(concat_name, block, input_cc, output_cc) ;
    
    for pp=1:length(net.layers)
        m=strcmp(net.layers(pp).inputs,input_cc2);
        if m
           net.setLayerInputs(net.layers(pp).name, {output_cc});
           sz_block=net.layers(pp).block.size;
           sz_name=net.vars((idxall(kk,1))).name;
           l = net.getLayerIndex(sz_name) ;
           sz_block_concat=net.layers(l).block.size;
           sz_block(4)=sz_block(4)+sz_block_concat(3);
           net.layers(pp).block.size=sz_block;
        end
    end
end
