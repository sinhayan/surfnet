function net = res_CVPRhandST_init(m,use_transformer, varargin)
switch m,
...
% 
% Meta parameters
net.meta.inputSize = [64 64 ninputs] ;
...
num_ST=2;
 if use_transformer
    ...
    st_num=1;
    imsz=[64,64];
    strST=['ST',num2str(st_num)];
    input_var='image';
    % ************************** localization network ****************************
    l_mp1 = dagnn.Pooling('method', 'max', 'poolSize', [2 2],'pad', 0, 'stride', 2);
    net.addLayer([strST,'l_mp1'], l_mp1, {input_var}, {[strST,'x1']});
    l_cnv1 = dagnn.Conv('size',[5 5 ninputs 20],'pad',0,'stride',1,'hasBias',true);
    net.addLayer([strST,'l_cnv1'], l_cnv1, {[strST,'x1']}, {[strST,'x2']}, {[strST,'lc1f'],[strST,'lc1b']});
    net.addLayer([strST,'l_re1'], dagnn.ReLU(), {[strST,'x2']}, {[strST,'x3']});

    l_mp2 = dagnn.Pooling('method', 'max', 'poolSize', [2 2],'pad', 0, 'stride', 2);
    net.addLayer([strST,'l_mp2'], l_mp2, {[strST,'x3']}, {[strST,'x4']});
    l_cnv2 = dagnn.Conv('size',[5 5 20 20],'pad',0,'stride',1,'hasBias',true);
    net.addLayer([strST,'l_cnv2'], l_cnv2, {[strST,'x4']}, {[strST,'x5']}, {[strST,'lc2f'],[strST,'lc2b']});
    net.addLayer([strST,'l_re2'], dagnn.ReLU(), {[strST,'x5']}, {[strST,'x6']});

    l_fc1 = dagnn.Conv('size',[10,10,20,50],'pad',0,'stride',1,'hasBias',true);
    net.addLayer([strST,'l_fc1'], l_fc1, {[strST,'x6']}, {[strST,'x7']}, {[strST,'lfcf'],[strST,'lfcb']});
    net.addLayer([strST,'l_re3'], dagnn.ReLU(), {[strST,'x7']}, {[strST,'x8']});

    % output affine transforms:
    l_out = dagnn.Conv('size',[1,1,50,6],'pad',0,'stride',1,'hasBias',true);
    net.addLayer([strST,'l_out'], l_out, {[strST,'x8']}, {[strST,'aff']}, {[strST,'lof'],[strST,'lob']});
    %***** NEED TO SET THE PARAMETERS OF THIS LAST LAYER TO OUTPUT IDENTITY *******

    % ************************** spatial transformer ******************************
    aff_grid = dagnn.AffineGridGenerator('Ho',imsz(1),'Wo',imsz(2));
    net.addLayer([strST,'aff'], aff_grid,{[strST,'aff']},{[strST,'grid']});

    sampler = dagnn.BilinearSampler();
    net.addLayer([strST,'samp'],sampler,{input_var,[strST,'grid']},{[strST,'xST']});
    % *****************************************************************************
  end
  
if use_transformer, in_name = [strST,'xST']; end


% First conv layer
nchannels=16;
block = dagnn.Conv('size',  [3 3 ninputs nchannels], 'hasBias', true, ...
                   'stride', 2, 'pad', [1 1 1 1]);%128
lName = 'conv0';
net.addLayer(lName, block, in_name, lName, {[lName '_f'], [lName '_b']});

add_layer_bn(net, nchannels, lName, 'bn0', 0.1);
block = dagnn.ReLU('leak',0);
net.addLayer('relu0',  block, 'bn0', 'relu0');


info.lastNumChannel = nchannels;
info.lastIdx = 0;

% Three groups of layers
info = add_group(opts.networkType, net, n, info, 3, 32, 2, opts);%64
info = add_group(opts.networkType, net, n, info, 3, 64, 2, opts);%32
info = add_group(opts.networkType, net, n, info, 3, 128, 2, opts); %16
info = add_group(opts.networkType, net, n, info, 3, 256, 2, opts); %8
% info = add_group(opts.networkType, net, n, info, 3, 512, 2, opts); %8
% Three groups of transpose layers

% info = add_groupT(opts.networkType, net, n, info, 3, 512, 0.5, opts);%16
info = add_groupT(opts.networkType, net, n, info, 3, 256, 0.5, opts);%16
info = add_groupT(opts.networkType, net, n, info, 3, 128, 0.5, opts);%32
info = add_groupT(opts.networkType, net, n, info, 3, 96, 0.5, opts); %64
info = add_groupT(opts.networkType, net, n, info, 3, 64, 0.5, opts); %128
info = add_groupT(opts.networkType, net, n, info, 3, 32, 0.5, opts); %256
...
if opts.reLUafterSum 
    ...
    block = dagnn.Pooling('poolSize', [1 1], 'method', 'avg', 'pad', 0, 'stride', 1);
...
if use_transformer, lName = [strST,'xST']; end
...
net.initParams();
  ...
  if use_transformer
    ...
    for st_num=1:num_ST
        strST=['ST',num2str(st_num)];
        f_prev = net.params(net.getParamIndex([strST,'lof'])).value;
        net.params(net.getParamIndex([strST,'lof'])).value = 0*f_prev;
        
        b_prev = 0*net.params(net.getParamIndex([strST,'lob'])).value;
        b_prev(1) = 1; b_prev(4) = 1;
        net.params(net.getParamIndex([strST,'lob'])).value = b_prev;
    end
  end
