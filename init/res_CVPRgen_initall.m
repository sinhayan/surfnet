function net = res_CVPRgen_initall(m,coord,sz_out,ninputs, varargin)
switch m,
...
net.meta.trainOpts.learningRate = [0.01*ones(1,5) 0.001*ones(1,5) 0.0001*ones(1,5)] ;
...
block = dagnn.Conv('size',  [1 1 4 128], 'hasBias', true, ...
    'stride', 2, 'pad', 0);
lName = 'convfc2';
...
elseif sz_out==64
    info = add_groupT(opts.networkType, net, n, info, 3, 64, 1, opts); %256
end
% info = add_groupT(opts.networkType, net, n, info, 3, 64, 0.5, opts); %256
...
elseif strcmpi(netType, 'resnet'), 
    if stride<1
        ...
        info = add_block_resT(net, info, [w-1 w-1 info.lastNumChannel ch], stride, true, opts);
    end
  for i=2:n, 
        if opts.bottleneck,
...
if stride < 1 || isFirst,
    ...
    block = dagnn.ConvTranspose('size',[2 2 ch f_size(3)], 'hasBias', false,'upsample',round(1/stride), ...
        'crop', 0);
    elseif stride==1
        block = dagnn.ConvTranspose('size',[1 1 ch f_size(3)], 'hasBias', false,'upsample',1, ...
        'crop', 0);     
    end
...
if opts.bottleneck,
    ...
    add_block_conv(net, sprintf('%d',info.lastIdx+1), sprintf('relu%d',info.lastIdx), ...
       ...
       add_block_conv(net, sprintf('%d',info.lastIdx+1), sprintf('relu%d',info.lastIdx), ...
        [f_size(1)+1 f_size(2)+1 info.lastNumChannel info.lastNumChannel], 1, opts);   
